...
function onSay(player, words, param)
	...
	if (player:getCoinsBalance() >= price) then
		player:setCoinsBalance(player:getCoinsBalance() - price)
		house:setOwnerGuid(player:getGuid())
		
		local rentPeriod = configManager.getString(configKeys.HOUSE_RENT_PERIOD)
	
		if rentPeriod ~= "never" then
			rentString = " Be sure to have " .. price .. " Tibia Coins in your bank account for the " .. rentPeriod .. " rent."
		else
			rentString = ""
		end
		
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You bought " .. house:getName(tile) .. " for " .. price .. " Tibia Coins from your Lioons OT account." .. rentString)
		return true
	else
		player:sendCancelMessage("You do not have enough Tibia Coins.")
		return false
	end
